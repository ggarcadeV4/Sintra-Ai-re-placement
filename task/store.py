...
def create_task(subject, description, active_form="", metadata=None) -> Task:
    with _lock:
        _load()
        task = Task(id=_next_id(), subject=subject, description=description,
...
def update_task(task_id, subject=None, description=None, status=None,
                active_form=None, owner=None, add_blocks=None,
                add_blocked_by=None, metadata=None) -> tuple[Task | None, list[str]]:
    with _lock:
        _load()
        ...
        if subject is not None and subject != task.subject:
            task.subject = subject; updated_fields.append("subject")
        if description is not None and description != task.description:
            task.description = description; updated_fields.append("description")
        if active_form is not None and active_form != task.active_form:
            task.active_form = active_form; updated_fields.append("active_form")
        if owner is not None and owner != task.owner:
            task.owner = owner; updated_fields.append("owner")
        if status is not None:
            ...
            if new_status is not None and new_status != task.status:
                task.status = new_status; updated_fields.append("status")
        if metadata is not None:
            for k, v in metadata.items():
                if v is None: task.metadata.pop(k, None)
